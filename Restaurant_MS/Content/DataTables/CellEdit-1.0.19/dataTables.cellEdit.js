...
jQuery.fn.dataTable.Api.register('MakeCellsEditable()', function (settings) {
    ...
    jQuery.fn.extend({
        ...
        updateEditableCell: function (callingElement) {
            ...
            var oldCellValue = '';
            // Update
            var newValue = $.trim(inputField.val());
    ...
    if (table != null) {
        ...
        $(table.body()).on('dblclick', 'td', function () {
            ...
            // hacer editable primera columna solo si es nuevo registro     --------------------------
            if (currentColumnIndex === 0 || (currentColumnIndex === 1 && !($(this).closest('tr').hasClass('bg-insert-color')))) {
                console.log('columnindex' + currentColumnIndex);
                return false;
            }
            ...
            if ((settings.columns && settings.columns.indexOf(currentColumnIndex) > -1) || (!settings.columns)) {
                ...
                if (!$(cell).find('input').length && !$(cell).find('select').length && !$(cell).find('textarea').length) {
                    ...
                    if (input.focus) {                       
                        var fldLength = $(input.html).val().length;
                        $('#cellEditable').focus();
                        $('#cellEditable')[0].setSelectionRange(fldLength, fldLength);
                    }
                }
            }
        });
...
function getInputHtml(currentColumnIndex, settings, oldValue) {
    ...
    switch (inputType) {
        ...
        case "datepicker-confirm":
            ...
            input.html = "<input id='cellEditable' type='text' name='date' class='datepick " + inputCss + "'   value='" + oldValue + "'></input> &nbsp;<a href='javascript:void(0);' class='" + confirmCss + "' onclick='$(this).updateEditableCell(this)'>Confirm</a> <a href='javascript:void(0);' class='" + cancelCss + "' onclick='$(this).cancelEditableCell(this)'>Cancel</a>";
        ...
        case "text-confirm": // text input w/ confirm
            input.html = "<input id='cellEditable' class='" + inputCss + "' value='" + oldValue + "'></input>&nbsp;<a href='javascript:void(0);' class='" + confirmCss + "' onclick='$(this).updateEditableCell(this)'>Confirm</a> <a href='javascript:void(0);' class='" + cancelCss + "' onclick='$(this).cancelEditableCell(this)'>Cancel</a> ";
            break;
        ...
        case "textarea-confirm":
            input.html = "<textarea id='cellEditable' class='" + inputCss + "'>" + oldValue + "</textarea><a href='javascript:void(0);' class='" + confirmCss + "' onclick='$(this).updateEditableCell(this)'>Confirm</a> <a href='javascript:void(0);' class='" + cancelCss + "' onclick='$(this).cancelEditableCell(this)'>Cancel</a>";
            break;
        default: // text input
            input.html = "<input id='cellEditable' class='" + inputCss + "' onfocusout='$(this).updateEditableCell(this)' value='" + oldValue + "'></input>";
            break;
    }
    return input;
}
